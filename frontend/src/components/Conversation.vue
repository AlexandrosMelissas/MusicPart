     ...
     <div v-for="message in getMessages" :key="message._id">
       <div class="message" v-bind:class="message.recipient == getMyProfile.owner._id ? 'not-my_message' : 'my_message'">
         <div v-if="message.sender" class="message_info">
          ...
          <p>{{ message.message }}</p>
          <ion-icon style="cursor:pointer;margin-top:1rem;font-size:2.5rem"  v-if="message.file" @click="playPauseSong" v-bind:data-source="'http://localhost:3000/api/message/' + message._id + '/file'" name="play"></ion-icon>
      </div>
    </div>
  </div>
</div>
